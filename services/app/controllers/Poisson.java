 ...
 */
public class Poisson extends AbstractController {
	...
	public static void poissonParId(Long id) {
		models.Poisson poisson = models.Poisson.find("byId", id)
				.<models.Poisson> first();
		if (poisson != null)
			render(poisson);
		else
			notFound();
	...
	public static void familles() {
		...
		if (famillePoissons != null)
			render(famillePoissons);
		else
			notFound();
	}

	public static void familleParId(Long id) {
		FamillePoisson famillePoisson = FamillePoisson.find("byId", id)
				.<FamillePoisson> first();
		if (famillePoisson != null)
			render(famillePoisson);
		else
			notFound();
	}
	...
	public static void poissonsDansFamille(Long id) {
		...
		if (famillePoisson != null)
			poissons = famillePoisson.poissons;
		if (poissons != null)
			render(poissons);
		else
			notFound();
